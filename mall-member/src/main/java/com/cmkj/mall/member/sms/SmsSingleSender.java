...
import com.cmkj.mall.member.util.ContextRepostiory;
...
public class SmsSingleSender {
	long appid;
	String appkey;
    String url = ContextRepostiory.smsurl;
	
	SmsSenderUtil util = new SmsSenderUtil();

	public SmsSingleSender(long appid, String appkey) throws Exception {
		this.appid = appid;
		this.appkey = appkey;
	}
...
*/
		...
		if (null == sign) {
			sign = ContextRepostiory.smssign;
		}
		if (null == extend) {
			extend = "";
		}		
		...
		String wholeUrl = String.format("%s?sdkappid=%d&random=%d", url, appid, random);
        HttpURLConnection conn = util.getPostHttpConn(wholeUrl);
//        Log4j.getlog().info("wholeUrl="+wholeUrl);
        OutputStreamWriter wr = new OutputStreamWriter(conn.getOutputStream(), "utf-8");
        wr.write(data.toString());
        wr.flush();
...
//        Log4j.getlog().info("result="+result);
        return result;
	}
}
